...
export const DoctorProvider = ({ children }) => {
  ...
  // Refresh cases from backend API
  const refreshCases = useCallback(async () => {
    try {
      setIsLoading(true);
      const backendCases = await fetchCases();
      if (backendCases && Array.isArray(backendCases) && backendCases.length > 0) {
        setCases(backendCases);
      }
    } catch (err) {
      console.warn('[DoctorContext] Could not fetch backend cases, keeping current state:', err.message);
    } finally {
      setIsLoading(false);
    }
  }, []);

  // Fetch cases on mount and poll gently every 8 seconds
  useEffect(() => {
    refreshCases();
    const interval = setInterval(refreshCases, 8000);
  ...
  const selectCase = useCallback(async (id) => {
    ...
    // Fetch fresh full case details from backend
    try {
      const detailed = await fetchCaseById(id);
      if (detailed) {
        setCases((prev) => prev.map((c) => (c.id === id || c.intakeId === id ? detailed : c)));
      }
    } catch (err) {
      // Non-critical, fallback to in-memory case
    }
  }, []);
  ...
  const selectedCase = useMemo(() => {
    return cases.find((c) => c.id === selectedCaseId || c.intakeId === selectedCaseId) || cases[0] || null;
  ...
  const updateDoctorNotes = useCallback((caseId, notes, provisionalDiagnosis) => {
    const reviewedAt = `${getFormattedDate()} at ${getFormattedTime()} (IST) by Dr. Ananya Sharma`;
    setCases((prevCases) =>
      prevCases.map((c) => {
        if (c.id === caseId || c.intakeId === caseId) {
          return {
            ...c,
            doctorNotes: notes,
    ...
    // Persist to backend
    saveDoctorNotesToBackend(caseId, { notes, provisionalDiagnosis, reviewStatus: 'Reviewed' });
  ...
  const markCaseReviewed = useCallback((caseId) => {
    ...
    // Persist to backend
    saveDoctorNotesToBackend(caseId, { reviewStatus: 'Reviewed' });
  ...
  const acceptCase = useCallback((caseId) => {
    ...
    // Persist to backend
    saveDoctorNotesToBackend(caseId, { reviewStatus: 'Accepted' });
